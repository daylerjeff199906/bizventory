...
async function manageProductVariants(
  ...
  try {
    ...
    const { data: existingVariants, error: fetchError }: SupabaseResponse<ProductVariantWithAttributes[]> = await supabase
    ...
    const variantsToUpsert: Partial<DatabaseProductVariant>[] = variantsData.map(variant => ({
    ...
    // Upsert de variantes
    const { data: createdVariants, error: variantsError }: SupabaseResponse<DatabaseProductVariant[]> = await supabase
      .from('product_variants')
    ...
    const variantsToDeactivate: ProductVariantWithAttributes[] = (existingVariants || [])
    ...
    if (variantsToDeactivate.length > 0) {
      const { error: deactivateError } = await supabase
        ...
        .in('id', variantsToDeactivate.map(v => v.id));
...
async function manageVariantAttributes(
  ...
  try {
    ...
    const { data: existingAttributes, error: fetchError }: SupabaseResponse<DatabaseProductVariantAttribute[]> = await supabase
    ...
    // Preparar datos para upsert
    const attributesToUpsert: Partial<DatabaseProductVariantAttribute>[] = attributes.map(attr => ({
    ...
    const attributesToDelete: DatabaseProductVariantAttribute[] = (existingAttributes || [])
    ...
    if (attributesToDelete.length > 0) {
      const { error: deleteError } = await supabase
        .from('product_variant_attributes')
        .delete()
        .in('id', attributesToDelete.map(a => a.id));
...
export async function handleProductVariantsUpdate(
  ...
  const { data: completeData, error: fetchError }: SupabaseResponse<ProductVariantWithAttributes[]> = await supabase
    .from('product_variants')
    .select(`
      *,
      product_variant_attributes (*)
