...
export async function sendWelcomeEmail({ firstName, email, password }: WelcomeEmailProps) {
    try {
        const { data, error } = await resend.emails.send({
            from: 'Bizventory <onboarding@resend.dev>', // Update this with your verified domain in production
            to: [email],
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL}/login" style="display: inline-block; background-color: #0070f3; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 10px;">Iniciar Sesión</a>
